...
def get_checkpoint_dir(group_name, data_percentage, selected_months, seed=42):
	months_sub = get_months_subdir(selected_months)
	d = os.path.join(path_config.get_checkpoint_root(), months_sub, f"{group_name}_{data_percentage}", f"seed_{seed}")
	os.makedirs(d, exist_ok=True)
	return d

def get_results_dir(selected_months, group_name=None):
	months_sub = get_months_subdir(selected_months)
	if group_name is not None:
		d = os.path.join("results", months_sub, group_name)
	else:
...
def compute_accuracy(gt_hls_tile, pred_hls_tile_avg, all_errors_hls_tile, hls_tile_n, paper_mask=None):
	...
	for idx in range(4):
		...
		n_pixels = len(errors)
		all_errors_hls_tile[hls_tile_n][idx] = {
			"sum_ae": np.sum(np.abs(errors)),
			"n_pixels": n_pixels,
		}
...
def _aggregate_tile_errors(all_errors_hls_tile, eval_loss, n_samples, pixel_weighted=False):
	"""Aggregate per-tile errors into per-date mean MAE and epoch loss.

	If pixel_weighted=False (default), each tile contributes one MAE value
	and we take an unweighted mean across tiles.
	If pixel_weighted=True, MAE is weighted by pixel count per tile.
	"""
	acc_dataset = {}
	for idx in range(4):
		if pixel_weighted:
			...
			for tile in all_errors_hls_tile:
				n = all_errors_hls_tile[tile][idx]["n_pixels"]
				if n > 0:
					total_ae += all_errors_hls_tile[tile][idx]["sum_ae"]
					total_pixels += n
			acc_dataset[idx] = float(total_ae / total_pixels) if total_pixels > 0 else 0.0
		else:
			per_tile = []
			for tile in all_errors_hls_tile:
				n = all_errors_hls_tile[tile][idx]["n_pixels"]
				if n > 0:
					per_tile.append(all_errors_hls_tile[tile][idx]["sum_ae"] / n)
			acc_dataset[idx] = float(np.mean(per_tile)) if per_tile else 0.0

	epoch_loss = eval_loss / n_samples
	return acc_dataset, all_errors_hls_tile, epoch_loss


def eval_data_loader(data_loader,model, device, tiles_paper_masks, loss_fn=None, pixel_weighted=False):
	...
	with torch.no_grad():
		for _,data in tqdm(enumerate(data_loader), total=len(data_loader)):
			...
			for gt_hls_tile, pred_hls_tile_avg, hls_tile_n in zip(ground_truth, pred_hls_tile_all, data["hls_tile_name"]): 
				...
				if tiles_paper_masks is not None: 
					...
					all_errors_hls_tile = compute_accuracy(gt_hls_tile, pred_hls_tile_avg, all_errors_hls_tile, hls_tile_n)

	return _aggregate_tile_errors(all_errors_hls_tile, eval_loss, len(data_loader.dataset))


def eval_data_loader_presto(data_loader, model, device, tiles_paper_masks, month=None, loss_fn=None, pixel_weighted=False):
	...
	return _aggregate_tile_errors(all_errors_hls_tile, eval_loss, len(data_loader.dataset), pixel_weighted=pixel_weighted)
...
def batched_sliding_window(model, image, crop_size, device, tile_size=330,
	...
	for start in range(0, len(all_rc), batch_size):
		...
		model_input = {
			"chip": crops,
			"temporal_coords": temporal_coords.unsqueeze(0).expand(n_crops, -1, -1).to(device),
			"location_coords": location_coords.unsqueeze(0).expand(n_crops, -1).to(device),
		}

		preds = model(model_input)  # (N, 4, cs', cs')
		preds = preds[:, :, :crop_size, :crop_size]
...
def eval_data_loader_crops(data_loader, model, device, tiles_paper_masks,
						   crop_size=48, stride=None, loss_fn=None, pixel_weighted=False):
	...
	with torch.no_grad():
		for _, data in tqdm(enumerate(data_loader), total=len(data_loader)):
			...
			image = data["image"]             # dict with chip, temporal_coords, location_coords
			gt = data["gt_mask"].to(device)    # (B, 4, H, W)
			tile_name = data["hls_tile_name"]

			chip = image["chip"]
			temporal_coords = image["temporal_coords"]
			location_coords = image["location_coords"]
			B = chip.size(0)

			for b in range(B):
				tc = temporal_coords[b]
				lc = location_coords[b]
				predictions = batched_sliding_window(
					model, chip[b:b+1], crop_size, device,
					tile_size=tile_size, stride=stride,
					temporal_coords=tc, location_coords=lc,
...
def eval_data_loader_presto_df(data_loader, model, device, tiles_paper_masks, month=None):
	"""Evaluate Presto model on full tiles and return per-pixel DataFrame."""
	...
	with torch.no_grad():
		for _, data in tqdm(enumerate(data_loader), total=len(data_loader)):
			...
			# Use raw image for n_missing_ts computation
			if "image_unprocessed" in data:
				img_raw = np.array(data["image_unprocessed"])
			elif isinstance(input, dict):
				img_raw = input["chip"].numpy()
			else:
				img_raw = input.numpy() if hasattr(input, "numpy") else np.array(input)
	...
	with torch.no_grad():
		for _, data in tqdm(enumerate(data_loader), total=len(data_loader)):
			...
			for b_idx in range(B):
				tc = temporal_coords[b_idx]
				lc = location_coords[b_idx]
				pred_tile = batched_sliding_window(
					model, chip[b_idx:b_idx+1], crop_size, device,
					tile_size=tile_size, stride=stride,
					temporal_coords=tc, location_coords=lc,
				)  # (4, 330, 330)
...
def save_checkpoint(model, optimizer, epoch, train_loss, val_loss, filename, selected_months=None):
	...
	# If model is torch.compiled, save the underlying module's state_dict
	# so checkpoints are compatible with non-compiled loading
...
def build_model(group, params, n_timesteps):
	...
	Returns:
		(model, crop_size) where crop_size is None for
	...
	if "presto" in group:
		...
		if "_timeloc-" in params and "_feed_timeloc-" not in params:
			timeloc_mode = parse_param(params, "timeloc", default=None, cast=str)
		else:
			timeloc_mode = None
		dropout = parse_param(params, "dropout", default=0.0, cast=float)
		...
		model = PrestoPhenologyModel(num_classes=4, freeze_encoder=freeze, input_mode="hls",
		                             ...
		                             use_ndvi=use_ndvi, pretrained=pretrained)
		return model, None

	# --- Paper-matched transformer (Tran et al. 2025) ---
	if "transformer_1d_paper" in group:
		from lib.models.transformer_1d_paper import TemporalTransformerPaper
		d_model = parse_param(params, "dm", default=64, cast=int)
		nhead = parse_param(params, "nhead", default=4, cast=int)
		num_layers = parse_param(params, "nl", default=3, cast=int)
		dropout = parse_param(params, "dropout", default=0.1, cast=float)
	...
	# Detect model size from group name (e.g., "...conv3d_tiny_crop32" -> "tiny")
	if "_tiny" in group:
		model_size = "tiny"
	elif "_100m" in group:
		model_size = "100m"
		...
		model_size = "300m"  # backwards-compat default
	with open(f'lib/models/prithvi_configs/prithvi_{model_size}.yaml', 'r') as f:
	...
	if "prithvi" in group:
		...
		concat_input = parse_param(params, "concat_input", default=True, cast=str2bool)
		dropout = parse_param(params, "dropout", default=0.0, cast=float)
		p_loc_drop = parse_param(params, "plocdrop", default=0.0, cast=float)
		model = PrithviPhenology(
			prithvi_config["pretrained_cfg"], None,
			n_classes=4, model_size=model_size,
			feed_timeloc=feed_timeloc, n_layers=n_layers,
			concat_input=concat_input,
			dropout=dropout, p_loc_drop=p_loc_drop,
		)
		return model, crop_size
