...
class CycleDatasetPixelsS2(Dataset):
	def __init__(self, data_dir, split, cache_path=None,
				 data_percentage=1.0, target_size=330, regenerate=False,
				 n_timesteps=12, file_suffix="", skip_normalization=True):
		"""
		Pixel-level dataset for S2 composites.
		...
		if not skip_normalization:
			self._get_means_stds()
			self._means_tensor = torch.tensor(self.means, dtype=torch.float32).view(1, -1)  # (1, 6)
			self._stds_tensor = torch.tensor(self.stds, dtype=torch.float32).view(1, -1)    # (1, 6)
