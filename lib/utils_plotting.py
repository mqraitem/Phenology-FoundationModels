...
def plot_methods_regions_split(
    ...
    date_order=("Greenup", "Maturity", "Senescence", "Dormancy", "Mean")
):
...
def results_file(split="test", selected_months=[3,6,9,12]):
	"""Load per-seed result CSVs per model.

	Uses the FIRST seed's DataFrame as the representative result (for
	functions that need a single DataFrame per model, like get_mae and
	add_region_to_results). The per-seed list is returned separately
	so callers can compute mean/std of MAEs across seeds.

	Returns:
		results: dict of first-seed DataFrames keyed by model name
		results_w_regions: same with region info added
		results_per_seed: dict of {model_key: [df_seed1, df_seed2, ...]}
	"""
	...
	for model_dir in os.listdir(base_dir):
		...
		# Collect all seed CSVs for this split
		seed_dfs = []
		for f in sorted(os.listdir(model_path)):
			if f.endswith(f"_{split}.csv") and f.startswith("seed_"):
				seed_dfs.append(pd.read_csv(os.path.join(model_path, f)))
...
def plot_performance_tiles(results_mae_df, methods= ["prithvi_pretrained_1.0_test", "transformer_1d_1.0_test"]):
    ...
    for idx, date in enumerate(date_names):
        ...
        #make xlabel bold
        axs[idx].set_xlabel(f"Date: {date}", fontsize=14, fontweight='bold')
